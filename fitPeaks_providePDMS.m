function [ peak_positions,param,fval ] = fitPeaks_providePDMS( x, y, Npeaks, method, peaks0, existing_spectrum, varargin )
...
ub=[max(x),10,max(y)*1e2, max(x),10,max(y)*1e2,    max(y)*1e2,max(y)*1e2];
lb(1:3:6)=param0_extended(1:3:6)-2;
ub(1:3:6)=param0_extended(1:3:6)+2;
