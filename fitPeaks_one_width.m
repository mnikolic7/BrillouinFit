function [ peak_positions,param,fval ] = fitPeaks_one_width( x, y, Npeaks, method, peaks0, varargin )
...
lb0=[1,        2,   1   ]; %pos, width, intensity
ub0=[max(x),   50,    max(y)*1e2];
lb=repmat(lb0,1,Npeaks);
ub=repmat(ub0,1,Npeaks);
lb(1:3:end)=param0(1:3:end)-2;
ub(1:3:end)=param0(1:3:end)+2;
lb(2:3:end)=param0(2:3:end);
ub(2:3:end)=param0(2:3:end);
